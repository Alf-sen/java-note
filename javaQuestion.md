...
* 线程阻塞方法
  * Tread.sleep()：当前线程调用，线程进入阻塞，但不释放对象锁，等到一定时间自动进入就绪状态。作用：给其他线程执行机会的最佳方式。
...
* Tread.yield()：当前线程调用该方法，使该线程放弃已经获取的cpu时间片，由运行状态进入就绪状态，让OS再次选择线程。作用：让同优先权的线程轮流执行，但是并不一定能实现，因为在此选择时，OS可能还是选择该线程。
