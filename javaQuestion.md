...
* hash索引：单值查询速度快，不适合范围查询。
* 红黑树和二叉树：数据多了，或者递增数据时，树很高，类似于链表查询。
* B树：不适合范围查询。
* B+树
  * 非叶子节点只存储索引，不存data数据。
  * 叶子节点存放所有的索引值和data数据。
  * 叶子节点采用双向链表连接，提高范围查询效率。

#### 52、MySQL两大存储引擎

Innodb和MyISAM。底层原理都是B+树。

Innodb是聚集索引，索引与数据放在一起；MyISAM是非聚集索引，索引与数据分开存储。

#### 53、主键索引与唯一索引区别

1. 唯一索引允许空值，只要索引列的所有值只出现一次即可，一个表可以有多个唯一索引。
2. 主键索引不允许有空值，一个表只允许有一个主键索引。
3. 主键索引一定是唯一索引，唯一索引不一定是主键索引。
4. 主键索引可以与外键索引相关联，确保数据一致性。
